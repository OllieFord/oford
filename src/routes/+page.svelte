<svelte:head>
	<title>Ollie Ford - Home</title>
</svelte:head>

<article class="prose md:prose-2xl leading-8">
	<h2 class=" text-slate-800/90 font-bold mb-4">
		Hi, I'm <span
			class="hover:text-violet-400 transition-colors ease-in-out duration-500 cursor-pointer"
			>Ollie!</span
		>
	</h2>
	<h2 class=" text-slate-800/60 font-normal mt-4">
		I design, build and operate <span
			class="font-base bg-gradient-to-r bg-clip-text  text-transparent 
            from-indigo-500 via-purple-500 to-indigo-500
            animate-text">machine learning</span
		> systems at scale.
	</h2>
	<div class="text-lg">
		<p>
            Currently, I work as a <span class="font-bold">Machine Learning Engineer</span> at <a class="text-indigo-600 " href="https://sympower.net/">Sympower</a>, shipping ML and forecasting systems to help customers participate on the energy flexibility market.
		</p>
		<p>
			I specialise in extracting knowledge from language <span class="font-bold">[NLP]</span>,
			understanding why systems make the decisons they do
			<span class="font-bold">[Interpretability]</span>
			and creating systems that augment and improve a users quality of life
			<span class="font-bold">[HCI]</span>.
		</p>
		<p>
			Outside of work, I enjoy cooking 👨‍🍳, coffee-shopping ☕, doggos 🐕 and working on side
			projects 🚀
		</p>
	</div>
</article>
